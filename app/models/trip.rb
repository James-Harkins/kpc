class Trip < ApplicationRecord
  ...
  def sort_by_calendar_admin
    ...
    nights.each do |night|
      ...
      calendar_date[:house_attendance_list] = night.golfer_nights.map {|golfer_night| golfer_night.golfer}
      
      rounds.where(date: night.date).each do |round|
        calendar_date[:round_attendance] = round.golfer_rounds.length
        calendar_date[:round_attendance_list] = round.golfer_rounds.map {|golfer_round| golfer_round.golfer}
      end
