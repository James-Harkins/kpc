class GolferTripFacade
  def self.create_new_golfer_trip(golfer, trip, params)
    ...
    if params[:full_trip]
      trip.nights.each {|night| golfer.golfer_nights.find_or_create_by!(night_id: night.id)}
      trip.rounds.each {|round| golfer.golfer_rounds.find_or_create_by!(round_id: round.id)}
    else
      ...
      params[:nights].select { |n| valid_night_ids.include?(n.to_s) }
                     .each   { |n| golfer.golfer_nights.find_or_create_by!(night_id: n) }
      params[:rounds].select { |r| valid_round_ids.include?(r.to_s) }
                     .each   { |r| golfer.golfer_rounds.find_or_create_by!(round_id: r) }
    end
